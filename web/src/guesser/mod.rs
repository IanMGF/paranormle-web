...
pub fn guesser() -> Html {
    ...
    let episode: UseStateHandle<Option<Episode>> = use_state(|| None);
    ...
    use_effect_with((), {
        ...
        move |_| {
            wasm_bindgen_futures::spawn_local(async move { 
                let episode_idx = daily_episode::get_day_episode().await.unwrap();
                episode.set(Some(episodes[episode_idx].clone()));
            });
        }
    });

    let Some(today_ep): Option<Rc<Episode>> = (*episode).clone().map(Rc::new) else {
        return html!{
            <>
                { "Carregando..." }
            </>
